...
function install_GCP_key_info() {
  ...
  echo "Installing GCP key info file."
  scp $IDENTITY $KEYS_DIRECTORY/$GCP_KEY_FILENAME $WORKING_AS@$INSTANCE_DNS_IPV4:/home/$SERVER_NAME/.keys
  ssh $IDENTITY $WORKING_AS@$INSTANCE_DNS_IPV4 "sudo chmod -R 660 /home/$SERVER_NAME/.keys/$GCP_KEY_FILENAME;"
}
